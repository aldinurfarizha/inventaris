...
class Laporan extends CI_Controller
{
	...
	{
		$data['data'] = $this->Global_model->getAllPembelianInventaris()->result();
	...
	public function insert_pembelian()
	{
		...
		if (checkPembelianExists($m, $y)) {
			echo "<script>alert('GAGAL. Sudah ada laporan pembelian pada bulan dan tahun tersebut.'); window.history.go(-1);</script>";
			return;
		}
		...
		$pelaksana_subdiv_logistik_detail = getEmployeeSimpeg(['pgw_id' => $id_pelaksana])->row();
		$nama_pelaksana_subdiv_logistik = $pelaksana_subdiv_logistik_detail->nama;
		$nik_pelaksana_subdiv_logistik = $pelaksana_subdiv_logistik_detail->nik;

		$data = array(
			'nama_kadiv_umum' => $nama_kadiv_umum,
			'nik_kadiv_umum' => $nik_kadiv_umum,
			'nama_kasubdiv_logistik' => $nama_kasub_aset,
			'nik_kasubdiv_logistik' => $nik_kasub_aset,
			'nama_pelaksana_subdiv_logistik' => $nama_pelaksana_subdiv_logistik,
			'nik_pelaksana_subdiv_logistik' => $nik_pelaksana_subdiv_logistik,
			'm' => $m,
			'y' => $y,
		);
		$pembelian_id = $this->Global_model->createPembelian($data);
		if (!$pembelian_id) {
	...
	public function delete_kir()
	{
		...
		$id_ruangan_kir = $this->input->post('id_ruangan_kir');
		$where = array(
			'id_kartu_inventaris' => $id
		);
		$this->Global_model->delete('kartu_inventaris', $where);
		$this->Global_model->delete('kartu_inventaris_barang', $where);
		return $this->output
			->set_content_type('application/json')
			->set_status_header(200)
