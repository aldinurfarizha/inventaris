...
class Master extends CI_Controller
{
	...
	public function dasar_perkiraan()
	{
		$data['title'] = "Master Dasar Perkiraan";
		$param = array(
			'deleted' => 0
		);
		$data['data'] = $this->Global_model->get_all('master_perkiraan_dasar')->result();
		$this->load->view('master/dasar_perkiraan', $data);
	}
	...
	public function add_perkiraan_dasar()
	{
		$kd_perkiraan_dasar = $this->input->post('kd_perkiraan_dasar');
		$nama_perkiraan_dasar = $this->input->post('nama_perkiraan_dasar');
		$data = array(
			'kd_perkiraan_dasar' => $kd_perkiraan_dasar,
			'nama_perkiraan_dasar' => $nama_perkiraan_dasar
		);
		if ($this->Global_model->is_kode_perkiraan_dasar_available($data['kd_perkiraan_dasar'])) {
			$this->Global_model->insert('master_perkiraan_dasar', $data);
			return $this->output
				->set_content_type('application/json')
				->set_status_header(200)
				->set_output(json_encode(array(
			...
			return $this->output
				...
				->set_output(json_encode(array(
					'status' => False,
					'messages' => 'Kode Perkiraan Dasar ini sudah di pakai,Gunakan Kode perkiraan dasar lain !'
				)));
		}
	}
	...
	public function edit_perkiraan_dasar()
	{
		$kd_perkiraan_dasar = $this->input->post('kd_perkiraan_dasar_edit');
		$nama_perkiraan_dasar = $this->input->post('nama_perkiraan_dasar_edit');
		$data = array(
			'nama_perkiraan_dasar' => $nama_perkiraan_dasar
		);
		$where = array(
		...
		if ($this->Global_model->update('master_perkiraan_dasar', $where, $data)) {
			return $this->output
				->set_content_type('application/json')
				->set_status_header(200)
				->set_output(json_encode(array(
